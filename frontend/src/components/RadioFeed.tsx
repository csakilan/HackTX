...
export function RadioFeed() {
  ...
  const filteredMessages = messages.filter(m => m.sender === 'ai' || m.sender === 'driver');
        ...
        <div ref={scrollRef} className="p-4 space-y-3">{/*  */}
          {filteredMessages.map((message, index) => (
            <motion.div
              ...
              className={`flex gap-3 ${message.sender === 'driver' ? 'justify-start' : 'justify-end'}`}
            >
              <div
              ...
              >
                ...
                {message.sender === 'ai' && message.confidence && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-[#00d2be] f1-text">{message.confidence}%</span>
                  </div>
                )}
              </div>
            </motion.div>
